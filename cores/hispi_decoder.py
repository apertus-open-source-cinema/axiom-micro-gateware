...
class HispiLane(Module):
    def __init__(self, hispi_bits):
        ...
        for bit_offset in range(0, hispi_bits):
            self.sync += If(word_buffer.slice(bit_offset + hispi_bits, 3 * hispi_bits) == sync_word,
                            offset.eq(bit_offset), self.aligned.eq(1))
...
class HispiPhy(Module):
    def __init__(self, num_lanes, buffer_depth, hispi_bits):
        ...
        self.comb += [self.outputs[i].eq(buffer[i].slice((lane_offsets[i] - 1) * hispi_bits, 3 * hispi_bits)) for i in
                      range(1, num_lanes)]
...
class HispiDecoder(Module):
    def __init__(self, num_lanes=4, buffer_depth=9, hispi_bits=12, output_bits=8):
        ...
        self.comb += [self.inputs[lane].eq(self.in_data[lane * hispi_bits:(lane + 1) * hispi_bits]) for lane in
                      range(num_lanes)]
        self.comb += [self.out[lane * output_bits:(lane + 1) * output_bits].eq(self.outputs[lane]) for lane in
                      range(num_lanes)]

        self.ios = {self.in_data} | \
                   {self.out, self.data_valid, self.frame_start}
        ...
        fsm.act("SYNC_CODE3",
                Case(sync_code(buffer[sync_lane]), {
                    ...
                    # embedded data, but we don't handle that yet
