...
class HispiBuffer(HispiBase):
    def __init__(self, data_in=None):
        buf = Buffer(word_count = self.config.buffersize, 
...
class HispiDataConverter(HispiBase):
    def __init__(self):
        ...
        self.sync += [self.data_out[lane][:-self.config.input_bits].eq(self.data_out[lane][self.config.input_bits:]) for lane in range(self.config.num_lanes)]

        for lane in range(self.config.num_lanes):
            for bit in range(self.config.input_bits):
                self.sync += self.data_out[lane][bit + self.config.hispi_bits - self.config.input_bits].eq(
                        self.config.lane_inversion_map[lane](self.data_in[self.config.num_lanes * bit + lane]))
    ...
    def __init__(self, lanes):
        ...
        fsm.act("SYNC_CODE3",
                Case(sync_code(), {
                    ...
                    0b000000001001: NextState("FILLER"),  # vblank, but we dont handle that yet (how would we handle that anyways?
...
class HispiRx(HispiBase):
    def __init__(self):
        converter = ClockDomainsRenamer("sys")(HispiDataConverter(self.config))
        ...
        self.submodules += ClockDomainsRenamer("hispi")(HispiWordAligner(config = self.config, bufs= bufs))

        decoder = ClockDomainsRenamer("hispi")(HispiDecoder(config = self.config, lanes = bufs))
